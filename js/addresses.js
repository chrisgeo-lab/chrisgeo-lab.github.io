...
function parseRows(headers, dataRows) {
  ...
  const results = dataRows.map(row => {
    ...
    const st = stateCol >= 0 ? (row[stateCol] || '') : '';
    const zip = zipCol >= 0 ? (row[zipCol] || '') : '';
    if (!city) {
      const parts = street.split(',');
      if (parts.length >= 2) { return {street: parts[0].trim(), city: parts[1].trim(), state: parts[2]?.trim() || st, zip, lat: null, lng: null}; }
    }
...
export function parsePastedText() {
  ...
  const addresses = lines.map(line => {
    ...
    if (parts.length >= 4) { street = parts[0]; city = parts[1]; st = parts[2]; zip = parts[3]; }
    else if (parts.length === 3) {
      street = parts[0]; city = parts[1];
      if (/^\d{5}(-\d{4})?$/.test(parts[2])) zip = parts[2];
      else st = parts[2];
    } else if (parts.length === 2) { street = parts[0]; city = parts[1]; }
    else { street = parts[0]; }
